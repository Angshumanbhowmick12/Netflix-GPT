

const VideoTitle = ({title,overview}) => {
  return (
    <div className='aspect-video pt-[20%] px-6 md:px-24 absolute text-white bg-gradient-to-r from-black'>
        <h1 className='text-2xl lg:text-6xl font-bold'>{title}</h1>
        <p className='hidden md:inline  lg:inline-block py-6 text-lg w-1/4'>{overview}</p>

        <div className="my-4 md:m-0">
            <button className="  bg-white text-black py-1 lg:py-4 px-3 lg:px-12 text-xl  rounded-lg hover:bg-opacity-80">
            ▶️ Play
            </button>

            <button className="hidden lg:inline-block mx-2  bg-gray-500 text-white p-4 px-12 text-xl bg-opacity-50 rounded-lg">
                More info
            </button>
        </div>
    </div>
  )
}

export default VideoTitle